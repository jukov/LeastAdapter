...
class LastAdapter(
...
) : RecyclerView.Adapter<LastAdapter.Holder>() {
    ...
    fun <M : Any, B: ViewBinding> map(clazz: Class<M>, type: Type<M, B>): LastAdapter =
        apply {
            require(type._onCreateView != null) { "onCreate not implemented" }
            if (hasStableIds()) {
                require(type._getItemId != null) { "StableIds requested, but getItemId for $clazz not implemented" }
            }
    ...
    override fun getItemViewType(position: Int): Int {
        val itemClass = list[position].javaClass
        return classToViewType[itemClass] ?: error("No viewType for $itemClass")
    }

    private fun getType(position: Int): Type<*, *> {
        return classToType[list[position].javaClass]
            ?: error("No Type for ${list[position].javaClass}")
    }
